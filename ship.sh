...
build() {
	if [[ $1 = "release" ]]; then
		if [[ $2 = "test" ]]; then
			...
			gcc tests/*.c src/*.c -DTEST=1 -Wextra -Os -o $TEST_RELEASE_OUT
		else
			echo "Building Release build"
			gcc src/*.c -Wextra -Os -o $RELEASE_OUT 
		fi
	else
		if [[ $2 = "test" ]]; then
			echo "Building Debug build"
			gcc tests/*.c src/*.c -DTEST=1 -DDEBUG=1 -o $TEST_DEBUG_OUT
		else
			echo "Building Debug build"
			gcc src/*.c -DDEBUG=1 -o $DEBUG_OUT
		fi
	fi
}
